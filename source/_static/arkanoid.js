...
(function ($, fg) {
	...
	$(function () {
		fg.resourceManager
			...
			.addAnimation('blue_brick', '_static/bricks.png', {type: fg.ANIMATION_VERTICAL, offsety: 0 * BRICK_HEIGHT, frameWidth: BRICK_WIDTH, frameHeight: BRICK_HEIGHT})
			.addAnimation('green_brick', '_static/bricks.png', {type: fg.ANIMATION_VERTICAL, offsety: 1 * BRICK_HEIGHT, frameWidth: BRICK_WIDTH, frameHeight: BRICK_HEIGHT})
			.addAnimation('brick', '_static/bricks.png', {type: fg.ANIMATION_VERTICAL, offsety: 2 * BRICK_HEIGHT, frameWidth: BRICK_WIDTH, frameHeight: BRICK_HEIGHT})
			.addAnimation('red_brick', '_static/bricks.png', {type: fg.ANIMATION_VERTICAL, offsety: 3 * BRICK_HEIGHT, frameWidth: BRICK_WIDTH, frameHeight: BRICK_HEIGHT})
			.addAnimation('ball', '_static/ball_large.png')
			.addAnimation('paddle', '_static/paddle_small.png')
		;
		...
		fg.startGame(function () {
			...
			for (i = 0; i < num_bricks; i += 1) {
				...
				fg.sprites[brick_name].userData = {energy: 1};
			...
			ball.userData = {
				state: STATE_MOVING,
				speed_x: BALL_SPEED * Math.cos(START_ANGLE),
				speed_y: BALL_SPEED * Math.sin(START_ANGLE)
			};

			$('#playground').mousedown(function () {
				if (ball.userData.state === STATE_MOVING) {
					ball.userData.state = STATE_STUCK;
				} else {
